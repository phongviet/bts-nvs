...
**Non-commercial determination (2026-07-16, team decision):** our participation in this
competition counts as **non-commercial research/evaluation use**. This unblocks every
non-commercial-research licence in the table below (INRIA 3DGS lineage incl. RaDe-GS/PGSR,
NVIDIA non-commercial, CC BY-NC). Recorded here as the team's determination, not as an
independently verified legal reading — if the competition's commercial terms change (e.g.
a prize assigning Viettel rights to submitted work), **every NC-licensed row below has to
be re-examined**, since that is exactly the condition that would void this call.
It does **not** speak to any other licence question: permissive-licence claims (SEA-RAFT,
mozjpeg, gsplat) still need their actual release confirmed, and the "not trained on
competition/BTS imagery" statement is required for every row regardless of licence.
...
| VGGT-1B (Meta AI, Wang et al. CVPR 2025) | Pseudo-point-cloud init arm (v2, arm c) | Wk2 | `facebook/VGGT-1B` on HuggingFace Hub via `VGGT.from_pretrained`; repo `github.com/facebookresearch/vggt` vendored read-only under `third_party/vggt/`, unmodified | Generic multi-view/SfM-style training corpora per the VGGT paper; not trained/finetuned on BTS/telecom imagery. Used strictly off-the-shelf for inference (no gradient updates) in `src/data_prep/build_vggt_init.py` | CC BY-NC 4.0 (Meta, non-commercial research) | [x] confirmed 2026-07-16 — competition use is non-commercial per the determination above, so CC BY-NC is compatible. |
| Difix3D+ / SD-Turbo | Diffusion "fixer" (post-hoc enhancer, exp015/016) | Wk3 | `nvidia/difix` on HuggingFace Hub via the vendored `DifixPipeline` (`src/enhancer/pipeline_difix.py`, from github.com/nv-tlabs/Difix3D, Apache-2.0 header) with `trust_remote_code=True` in `src/enhancer/run_difix.py`; single-step SD-Turbo-style img2img backbone. LoRA finetune (peft, UNet only) in `src/enhancer/train_difix_lora.py` | Generic image corpora per the Difix3D+ paper / SD-Turbo lineage; not BTS. **LoRA finetuned ONLY on our own (render, real) pairs** built from the organizers' provided train images by `src/enhancer/build_enhancer_pairs.py` (manifest.csv is the provenance record; no external-scene pooling per the v2 rule above) | Dual: NVIDIA non-commercial license (Work restricted to "research or evaluation purposes only") + Stability AI Community License (SD-Turbo lineage; research/non-commercial OK, outputs subject to terms) — read from `nvidia/difix` LICENSE.txt 2026-07-09 | [x] confirmed 2026-07-09 — research/eval use OK. **Moot for submissions: exp015 dropped, exp016 skipped (gate failed Jul 9); no difix output ships.** If ever revisited, the non-commercial precondition is now settled (determination above, 2026-07-16) — exp015/016 remain dropped on MERIT (gate failed Jul 9), not on licence. |
| RaDe-GS (arXiv 2406.01467) / PGSR (alternate arm) | exp041 depth-only A/B: per-train-view depth for the DIBR occlusion z-test (`Analysis/04 --depth-source`, imported via `Analysis/18_import_depth.py`) | Wk5 (Wave 2) | **NOT YET RUN.** Their repo, unmodified, run STANDALONE on our own COLMAP + the organisers' provided train images; we import only the exported depth ARRAY, never their code into ours. **Not a pretrained model at all** — it is trained from scratch, by us, on provided data only, so the "generic pretrained" framing does not apply and there is no foreign training corpus to vet | n/a — no pretraining; trained only on the organisers' provided BTS train images + provided COLMAP, which the rules explicitly allow | Non-commercial research (inherits the INRIA 3DGS lineage; CONFIRM the exact terms of the RaDe-GS release when the repo is first cloned) | [x] non-commercial precondition settled 2026-07-16 (determination above) — [ ] still confirm the release's own terms at clone time. Training a separate 3DGS-family model on provided data is squarely within the rules; the only licence surface is running their code. |
| SEA-RAFT (Wang et al., ECCV 2024 Oral, arXiv 2405.14793) | Optical flow for exp039 flow-residual alignment of warped neighbour views before the DIBR guard/blend (`Analysis/17_flow_align.py`, backend `searaft`) | Wk5 (Wave 1) | **NOT YET USED — no checkpoint vendored.** Intended: official `princeton-vl/SEA-RAFT` release checkpoint (Tartan-C / Spring / Sintel-trained), vendored read-only under `third_party/sea-raft/`, inference only (no gradient updates, no finetuning). The DEFAULT backend `dis` is `cv2.DISOpticalFlow` — a CLASSICAL algorithm with NO pretrained weights, so it needs no provenance row at all | Generic optical-flow corpora (TartanAir / FlyingChairs / FlyingThings / Sintel / KITTI per the paper); not trained/finetuned on competition or BTS/telecom imagery | BSD-3-Clause (per the SEA-RAFT repo, CONFIRM against the release before shipping) | [ ] **BLOCKS exp039 shipping with `--flow-align searaft`.** Confirm license + exact checkpoint name/URL when the ckpt is first vendored. No confirmation needed to ship the `dis` backend (weightless). |
...
- [ ] Fill the pretrained-model provenance table as each model is first used (VGGT/Depth Anything V2/SAM in Week 2; Difix3D+/SD-Turbo in Week 3; SEA-RAFT in Week 5 if the `searaft` flow backend is enabled).
...
- [ ] Re-examine every NC-licensed row if the competition's commercial terms turn out to differ from the 2026-07-16 non-commercial determination above (prize terms assigning rights to submitted work would be the trigger).

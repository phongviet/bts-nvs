...
| exp009 | perceptual LPIPS(VGG) fine-tune, +7k iters, w ∈ {0.05, 0.1} | — | — | — | ⬜ **ready on Kaggle** | `kaggle/kaggle-exp009-perceptual-ft.ipynb` + `kaggle-upload-exp009-tierA2.zip` (2.8 GB, ships antialiased 30k base ckpts under `exp009_base/`). **Priority arm post-calibration: the loss term optimizes 0.4 of the actual grader score (VGG confirmed).** exp013 tpw rides along (hcm0034). Kill if PSNR −0.3 dB. ~2.5–3.5 h on T4×2, keeps ckpts. |
...
| exp013 | test-pose-weighted train sampling | — | — | — | ⬜ **rides exp009 notebook** | Stretch; weights pre-verified non-degenerate (2.0× range). Adopt at ≥ +0.002 on hcm0034. |
| exp014 | camera-optimizer + drift guard | — | — | — | ⬜ local | Stretch. Public-GT Score must improve, else off permanently. |
| exp015 | Difix3D+ off-the-shelf | — | — | — | ⬜ **ready on Kaggle** | `kaggle/kaggle-exp015-difix.ipynb` + `kaggle-upload-exp015-difix.zip` (115 MB: locked-config pilot renders + test GT). Pure inference, ~1 h, 1 GPU. Verifies resolution round-trip/VRAM/latency; scores pre/post vgg+50 → `week3_enhancer_ablation.csv`; QA crops in output. **Gates exp016**: ≥+0.003 → LoRA full priority; ≤0 both pilots → skip LoRA. |

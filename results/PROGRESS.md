...
| exp005 | locked (dense + antialiased) → 8 private scenes | n/a (private) | — | ⬜ ready to submit | ✅ | **Fleet complete (Jul 9): all 8 scenes re-integrated** (3 local + 5 rented via `week2_train_output.zip`), render counts/sizes verified 8/8 vs test_poses.csv. **Two zips packaged:** clean `submissions/phase1/exp005_antialiased_dense_results/submission_round1.zip` (datapoint #3 — isolates antialiased delta, local prediction +0.0016) and `exp005_antialiased_dense_pp_results/…` (exp011 postproc applied — submit as #4 after the 600 s cooldown; isolates postproc delta, local prediction +0.0039). P2 submits in that order. |
...
| exp011 | post-process + encoding sweep (JPEG quality / unsharp / denoise) | **+0.0039 mean, won all 5 public scenes** | — | — | 🟢 | **Adopt `unsharp_r1_p50 + jpeg98` in the packaging path** (`apply_postprocess.py`). Sharp optimum (p100/r2 lose); encoder axis flat ≥jpeg90; round-trip check clean. Full grid in `results/week3_postproc_ablation.csv` (7 ops × 6 encoders × 5 scenes, exp002 renders). Re-verify on next submission (delta-transfer check #2). |

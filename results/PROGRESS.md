...
| exp007 | bilateral grid A/B | hcm0034 control 0.6548 (vgg+50) ✅ | — | — | ⬜ **queued on Kaggle** | Whole experiment moved to `kaggle/kaggle-exp007-008-tierA.ipynb` + `kaggle-upload-exp007-008.zip` (Jul 9) after bilateral thrashed the 6 GB card (3.2 s/iter, 14×, killed at 59%). Session runs: HCM0181 control + bilateral ×2 + exp008 masked ×2, ~4–6 h on T4×2. Adopt at ≥ +0.002 mean vgg+50. |
| exp008 | transient-mask A/B (person+vehicle out of loss) | — | — | — | ⬜ **queued on Kaggle** (same notebook as exp007) | Expect win concentrated on HCM0181 → likely per-scene override rather than global adopt. Masks built + QA'd Jul 8 (slightly over-eager on static red objects); shipped in the upload zip; yaml passes `--masks-path masks` (dataparser_args — no auto-pickup, fixed Jul 9). |

...
| exp007 | bilateral grid A/B | controls: hcm0034 0.7243 ✅ (reproduces exp004 antialiased 0.7242); HCM0181 🟡 running | — | — | 🟡 **bilateral cells RE-SCOPED to rented (Jul 9)** | Bilateral grid doesn't fit local: 3.2 s/iter vs 0.23 control (14×, VRAM at 5.1/6 GB = thrashing), ETA ~11 h/cell — killed at 59%. Run `run_sweep.py --sweep exp007…yaml --variants bilateral` on a rented GPU (ride along with exp006 session 2). Controls run local (vanilla, fast). |
